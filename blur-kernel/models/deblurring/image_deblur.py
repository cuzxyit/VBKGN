...
from models.dips import ImageDIP, KernelDIP,SRCNN
...
class ImageDeblur:
    def __init__(self, opt):
        ...
        self.kernel_wizard = TwoHeadsNetwork(K=opt["EDnet"]["K"],nz=opt["EDnet"]["nz"]).cuda()  # KernelWizard(opt["KernelWizard"]).cuda()
        self.kernel_wizard.load_state_dict(torch.load(opt["KernelWizard"]["pretrained"]))
    ...
    def reset_optimizers(self):
        ...
        self.optimizerS = torch.optim.Adam(self.SR.parameters(), lr=self.opt["s_lr"])
    ...
    def prepare_DIPs(self):
        ...
        self.SR=SRCNN(upscale_factor=3).cuda()
