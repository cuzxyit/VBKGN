...
def forward_reblurce(sharp_estimated, kernels, masks, size='valid', padding_mode='reflect',
                   manage_saturated_pixels=True, max_value=0.5):
    sharp_n=sharp_estimated
    n_kernels = kernels.size(1)
    K = kernels.size(2)
    N = sharp_estimated.size(0)
    C = sharp_estimated.size(1)
    ...
    # conv_output0_bu = torch.ones(N, 19, H, W).cuda()

    mask0=masks[:,:6,:,:]
    mask1=masks[:, :12, :, :]
    mask2=masks[:, :18, :, :]

    n_kernels0 = kernel0.size(1)
    n_kernels1 = kernel1.size(1)
    n_kernels2 = kernel2.size(1)

    output_reblurred = torch.empty(N, n_kernels, C, H, W).cuda()


    output_reblurred0 = torch.empty(N, n_kernels0, C, H, W).cuda()
    output_reblurred1 = torch.empty(N, n_kernels1, C, H, W).cuda()
    output_reblurred2 = torch.empty(N, n_kernels2, C, H, W).cuda()
    ...
    for num in range(N):  # print('n = ',n)
        for c in range(C):
            # print('gt padded one channel shape: ', gt_n_padded_c.shape)
            a = kernel0[num][:, np.newaxis, :, :]
            conv_output0 = F.conv2d(sharp_estimated[num:num + 1, c:c + 1, :, :],
                                       kernel0[num][:, np.newaxis, :, :])

            # print('conv output shape: ', conv_output.shape)
            # conv_output0=torch.cat((conv_output0, conv_output0_bu), dim=1)

            output_reblurred0[num:num + 1, :, c, :, :] = conv_output0*mask0[num:num + 1] #* mask0[num:num + 1] **(1.0/2.2)
            del conv_output0
    ...
    for num in range(N):  # print('n = ',n)
        for c in range(C):
            ...
            conv_output1 = F.conv2d(sharp_estimated[num:num + 1, c:c + 1, :, :],
                                       kernel1[num][:, np.newaxis, :, :])

            # print('conv output shape: ', conv_output.shape)
            output_reblurred1[num:num + 1, :, c, :, :] = conv_output1* mask1[num:num + 1]
            del conv_output1
    ...
    for num in range(N):  # print('n = ',n)
        for c in range(C):
            ...
            conv_output2 = F.conv2d(sharp_estimated[num:num + 1, c:c + 1, :, :],
                                       kernel2[num][:, np.newaxis, :, :])

            # print('conv output shape: ', conv_output.shape)
            output_reblurred2[num:num + 1, :, c, :, :] = conv_output2 * mask2[num:num + 1]
            del conv_output2

    # print('reblur_image shape before sum:', reblurred_images.shape)
    output_reblurred = torch.sum(output_reblurred, (1))

    # output_reblurred0 = torch.sum(output_reblurred0, (1))
    # # torch.cat((output_reblurred0, conv_output0_bu), dim=1)
    # # output_reblurred0= output_reblurred0**(8)
    # output_reblurred1 = torch.sum(output_reblurred1, (1))
    # # output_reblurred1 = output_reblurred1**(1.0/1.5)
    # output_reblurred2 = torch.sum(output_reblurred2, (1))
    # output_reblurred2 = output_reblurred2**(1.0/1.5)

    # sharp_n= apply_saturation_function(sharp_n, max_value)
    if manage_saturated_pixels:
        output_reblurred = apply_saturation_function(output_reblurred, max_value)


    return output_reblurred
def forward_reblur(sharp_estimated, kernels, masks, size='valid', padding_mode='reflect',
                   manage_saturated_pixels=True, max_value=0.5):
    n_kernels = kernels.size(1)
    K = kernels.size(2)
    N = sharp_estimated.size(0)
...
def compute_gradient_loss(blur_image, sharp_image, kernels, masks):
    ...
    blurry_y = blur_image[:, :, 1:, :] - blur_image[:, :, :-1, :]
    blurry_x = blur_image[:, :, :, 1:] - blur_image[:, :, :, :-1]
    reblurred_images_y = forward_reblur( gt_sharp_y, kernels, masks, size='same')
    reblurred_images_x = forward_reblur( gt_sharp_x, kernels, masks,size='same')
