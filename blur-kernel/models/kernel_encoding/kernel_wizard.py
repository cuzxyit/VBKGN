...
class KernelWizard(nn.Module):
    def __init__(self, opt):
        ...
        # upsampling
        recon_trunk.append(nn.Conv2d(nf, nf * 4, 3, 1, 1, bias=True))   #64
        recon_trunk.append(nn.PixelShuffle(2))    #128
        recon_trunk.append(lrelu)
        recon_trunk.append(nn.Conv2d(nf, 64 * 4, 3, 1, 1, bias=True)) #128
        recon_trunk.append(nn.PixelShuffle(2)) #256
        recon_trunk.append(lrelu)
        recon_trunk.append(nn.Conv2d(64, 64, 3, 1, 1, bias=True))#256
        recon_trunk.append(lrelu)
    ...
    def adaptKernel(self, x_sharp, kernel):
        ...
        x_sharp = self.feature_extractor(x_sharp)#特征提取
