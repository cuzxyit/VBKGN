...
class TwoHeadsNetwork(nn.Module):
    def __init__(self, K=9,nz=200, blur_kernel_size=33, bilinear=False,
                 no_softmax=False):
        ...
        self.kernels_end_b = nn.Sequential(
            View((-1, self.K*self.blur_kernel_size * self.blur_kernel_size)),
            nn.Linear(self.K*self.blur_kernel_size * self.blur_kernel_size, nz * 2),
        )
        self.kernels_end_bk = nn.Sequential(
            nn.Linear(nz ,self.K * self.blur_kernel_size * self.blur_kernel_size, ),
            View((-1, self.K * self.blur_kernel_size * self.blur_kernel_size)),
    ...
    def sample (self, input):
        return  self.kernels_end_bk(input)
    def forward(self, x):
        #Encoder
        if x.shape[1]==3:
            x1 = self.inc_rgb(x)
        ...
        z1 = self.kernels_end_b(k)
        mu = z1[:, :self.nz]
        logvar = z1[:, self.nz:]
        z=self.reparametrize(mu,logvar)
        k=self.kernels_end_bk(z)
        k = k.view(N, self.K, self.blur_kernel_size * self.blur_kernel_size)


        # z = self.reparametrize(logvar)
        if self.no_softmax:
